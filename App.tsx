...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (!geminiApiKey) {
      ...
      setError("Kunci API Gemini tidak dikonfigurasi. Fitur AI akan dinonaktifkan.");
    }
  }, [geminiApiKey]);
  ...
  const getTaskAdvice = useCallback(async (task: Task): Promise<string> => {
    ...
    // Per-task loading state is managed within TaskItem.tsx, so global setIsLoading is not strictly needed here
    // setIsLoading(true); 
    setError(null);
    try {
      const advice = await getTaskAdviceFromAI(task, geminiApiKey);
      // Update the task with new advice if needed, or let TaskItem handle display
      // For now, just return it for TaskItem to display.
      return advice;
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 p-4 sm:p-8 text-slate-100 flex flex-col items-center">
      ...
      {apiKeyMissing && (
         <div className="w-full max-w-3xl p-4 mb-6 bg-yellow-500/20 text-yellow-300 border border-yellow-600 rounded-lg flex items-center" role="alert">
            ...
            <span>Kunci API Gemini tidak ada. Fitur AI dinonaktifkan. Harap atur variabel lingkungan API_KEY.</span>
        </div>
      )}
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6 mt-8">
        {Object.values(Quadrant).map(quadrantId => (
          <QuadrantColumn
            ...
            isLoadingAiAdvice={false} // TaskItem handles its own spinner
            isFull={tasks.filter(t => t.quadrant === quadrantId).length >= TASK_LIMIT_PER_QUADRANT}
          />
        ))}
      </div>
      
      {!apiKeyMissing && tasks.length > 0 && (
